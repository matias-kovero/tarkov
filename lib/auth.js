  ...
  async login(client, email, password, captcha, hwid) {
      try {
        ...
        let response = await client({
          url: `https://${LAUNCHER_ENDPOINT}/launcher/login?launcherVersion=${LAUNCHER_VERSION}&branch=live`,
          type: 'post',
          headers: {
            'Content-Type': 'application/json',
            'User-Agent': `BSG Launcher ${LAUNCHER_VERSION}`,
            'Host': LAUNCHER_ENDPOINT,
            //'Cookie': 'PHPSESSID=XXXXXXXXXXXXXXXXXXXXXX',
          },
          gzip: true,
          encoding: null,
          json: new LoginRequest(email, md5pass, hwid, captcha),
          transform: body => JSON.parse(pako.inflate(body, { to: 'string' }))
        }); 
        if(!response.err == 0) return response.errmsg;
        else return new Auth(response);
      } catch (err) {
...
//======================
function send_info(client, options, data) {
...
function handle_auth_error(obj) {
  if(!isNaN(obj.err) && !obj.err === 0) {
    switch(obj.err) {
      ...
      case 211: return 'BadTowFactorCode';
